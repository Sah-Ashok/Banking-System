...
export default function WithdrawForm({ accountId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("http://localhost:3001/api/accounts/withdraw", {
        method: "POST",
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex gap-4">
        <button
          type="submit"
          disabled={loading}
          className="flex-1 bg-red-500 hover:bg-red-600 disabled:bg-red-800 text-white py-3 rounded-lg font-semibold transition"
        >
